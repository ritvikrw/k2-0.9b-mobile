...
Java_com_arm_aichat_internal_InferenceEngineImpl_load(JNIEnv *env, jobject, jstring jmodel_path) {
    ...
    // Disable mmap to prevent SIGBUS on corrupted/partial files.
    // This forces the model to be loaded into memory, which is safer.
...
static std::string chat_add_and_format(const std::string &role, const std::string &content) {
    // Skip native chat formatting as it's handled in Kotlin.
    // We just keep track of the message.
    common_chat_msg new_msg;
    new_msg.role = role;
    new_msg.content = content;
    chat_msgs.push_back(new_msg);
    LOGi("%s: Added %s message (skipped formatting)", __func__, role.c_str());
    return content;
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processSystemPrompt(
    ...
    // Skip native chat templates for now as they cause instability/crashes with some models.
    const bool has_chat_template = false;
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPrompt(
    ...
    // Skip native chat templates as they cause instability/crashes with some models.
    // Our Kotlin layer already formats the prompt correctly.
    const bool use_native_template = false;

    env->ReleaseStringUTFChars(juser_prompt, user_prompt);

    // Decode formatted user prompts
    auto user_tokens = common_tokenize(g_context, formatted_user_prompt, use_native_template, use_native_template);
